...
The system SHALL save the captured photo at the SD root under the name `IMG_NNNNN.JPG` (N zero-padded to 5 digits). The SD SHALL be cleanly unmounted after the write.

#### Scenario: Successful save
- **WHEN** the JPEG is captured and the counter is incremented
- **THEN** `IMG_NNNNN.JPG` is created on the SD with the full JPEG content
